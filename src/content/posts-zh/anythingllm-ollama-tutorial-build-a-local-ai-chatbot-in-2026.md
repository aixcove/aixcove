...
<p>工具背景可参考<a href="/zh/listing/anythingllm/">AnythingLLM列表</a>、<a href="/zh/listing/openclaw/">OpenClaw列表</a>和<a href="/zh/listing/dify/">Dify列表</a>。</p>
<h2>你要构建什么</h2>
...
<p>较小的本地模型适合快速总结、简单分类和基础问答。较大的模型通常回答更好，但需要更多内存，在弱硬件上可能感觉缓慢。对于日常团队使用，预算要考虑到一台能保持在线并运行你真正想要的模型的电脑或服务器，而不只是演示中能跑的那个。</p><p>这套方案最适合单人操作者、注重隐私的团队、测试本地 RAG 的开发者，以及那些内部文档有用但敏感度不足以支撑完整企业级 AI 平台的企业。如果你需要审计审批、细粒度企业权限、保证可用性，或跨多个系统的复杂工作流自动化，它就显得不足。做这类决策时，可以在 <a href="/zh/category/ai-tools-comparisons/">AI 工具对比</a> 中比较不同工具。</p>
<h2>前置条件</h2>
<p>开始之前，请确认你具备以下条件：</p>
<ul>
...
<p>Docker 版本更适合需要他人访问的服务，或部署在服务器上的场景。开始前有一个网络细节：如果 AnythingLLM 跑在 Docker 里而 Ollama 跑在宿主机上，容器内的 <code>127.0.0.1</code> 指向容器自身，基础URL要改成宿主机地址（Docker Desktop for Mac/Windows 通常用 <code>host.docker.internal</code>；Linux 用宿主机IP或 docker 网桥网关）。当你重视可重复性和可用性时，用 Docker；当你今天就想得到有用答案时，用桌面版。</p>
<h2>步骤 3：将 AnythingLLM 连接到 Ollama</h2><p>在AnythingLLM内部，打开模型配置区域，选择Ollama作为LLM提供商。关键字段有两个：Ollama基础URL和模型名称。默认设置下，基础URL填<code>http://127.0.0.1:11434</code>（Ollama官方连接文档给出的默认地址）。模型名称必须与你之前拉取的完全一致，比如<code>llama3.2</code>——不确定确切标签时，先运行<code>ollama list</code>查看。保存设置后，先发一条普通聊天消息，确认模型能回复，再去添加文档。</p>
<ul>
...
<h2>第 6.5 步：用固定测试集验证</h2>
<p>在信任这个工作区之前，先用一小套固定材料和问题做验证——大约十分钟，把"看起来能用"变成"这五个问题它都能答对"。</p>
<ul>
<li><strong>选两三份小文档</strong>（每份几页即可），主题必须是你自己熟悉的。</li>
<li><strong>提前写下三个问题：</strong>一个文档能明确回答的（记下正确答案和出处文件/章节）、一个文档只能部分回答的、一个文档完全没有涉及的。</li>
<li><strong>预期结果：</strong>第一个问题得到正确答案且引用来源正确；第二个问题得到反映部分来源的谨慎回答；第三个问题得到明确的"文档未涉及"，而不是自信的编造。</li>
</ul>
<p>如果第三个问题得到了自信的虚构答案，回到第 6 步收紧系统提示词并重测。这套三问题用例也是你以后更换模型、嵌入器或提示词后的回归测试。</p>
...
<p>如果您的主要目标是跨消息渠道和定时任务的长期个人助理，<a href="/zh/listing/openclaw/">OpenClaw</a> 可能更合适。如果您的目标是为业务团队打包 AI 应用和工作流，<a href="/zh/listing/dify/">Dify</a> 值得一看。AnythingLLM 介于两者之间：作为私有知识工作区最强，代理功能在需要时可用。</p>
<h2>常见问题</h2>
...
<p>更多实操教程，浏览<a href="/zh/category/ai-tutorials-how-tos/">AI教程与操作指南</a>。</p>
